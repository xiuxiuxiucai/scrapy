...
def get_product():
    ...
    # 选择国省道扬尘
    driver.find_element_by_xpath("//li[@onclick=\"ShowTab('tab-05',this)\"]").click()
    # 全选站点
    driver.find_element_by_xpath("//table[@id='tab-05-stationList']/thead/tr/th[@class='text-center']/input").click()
    # 确定
    driver.find_element_by_xpath("//input[@value='确定']").click()
    time.sleep(2)

    # 修改查询时间
    set_query_time("01", str(calendar.monthrange(year, month)[1]), True)
    # 修改查询项目
...
def get_excel():
    # 创建一个工作簿并添加一张工作表
    workbook = xlsxwriter.Workbook("先河路长制VOC" + str(month) + "月份数据.xlsx")
    ...
    worksheet.write(0, 5, "VOC")
    worksheet.write(0, 6, "TP")
    worksheet.write(0, 7, "TD")

    # 获取数据并写入excel
    hour_number = re_parse_product("VOC 小时数据,", worksheet, 1)
    ...
    # 获取数据并写入excel
    day_number = re_parse_product("VOC 天数据,", worksheet, 1)
